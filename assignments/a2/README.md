...
**The three demos** we used in class, `demo-plcc-parse`, `demo-plcc-rep`,
...
you saw in `List.py` in the `demo-plcc-rep` demo — and put the **type** of
each attribute in a comment beside it, since Python will not say. For the
sanddemo's `<ListTail:Some> ::= COMMA <NUM> <ListTail>` that would be:
