# CS 373 - Demo: `plcc-rep`, the semantic section, and meaning
...
The parse demo ended with a parser that says *yes, `1, 2, 3` is a `List`*.
That is everything the parser knows. This demo is about the next question:
...
If you have not done the parse demo yet, do that one first:
`demos/demo-plcc-parse`. The grammar here is the one from there.
...
Same three reasons as the other demos: `-s sum.plcc` is a relative path,
...
Same tree as the parse demo. `plcc-rep` is that tree, *plus* your code.
...
The parse demo said a grammar *defines classes*. Look at them:
...
> drawing bug as the parse demo, triggered by the empty alternative. The
...
`rep.plcc` writes the grammar in one line, as the parse demo did:
