# CS 373 - Demo: `V0`, the first language
...
The three demos before this one built up a specification one section at a
...
demo is to be able to read `V0` whole — all three sections, and the classes
they turn into — before anything gets added.

If you have not done the earlier demos, do `demo-plcc-rep` first. Every
...
exactly as it did with whitespace in the first demo. Prove it:
...
`_fields`; the earlier demos said *field*. Same thing.)

That last row is the new thing. Four `Prim` classes, and none of them holds
anything. Ask the question from the parse demo: *how many lexemes could
